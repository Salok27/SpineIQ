...
/** Day-counts at which a streak crosses a celebrated milestone. */
object StreakMilestones {
    val DAYS: Set<Int> = setOf(3, 7, 14, 30)
}
...
/**
 * Pure streak rules. A streak-qualifying event is a daily check-in, a completed
 * ritual, or an assessment completion (per-step saves are excluded — they can
 * be repeated by back-navigating within one assessment). Day boundaries use the
 * device-local calendar date; travel across timezones can at worst gain or lose
 * one day, accepted for Phase 1.
 *
 * Streak breaks are applied lazily: nothing runs at midnight. Writes go through
 * [advance] on the next qualifying event, and reads use [effectiveStreak] so a
 * stale stored value displays as 0.
 */
object StreakLogic {
    ...
    fun advance(
        ...
        val crossed = StreakMilestones.DAYS
